...
import { ArrowLeft, Search, Filter, Mail, Phone, Calendar, Check, ChevronDown, Copy, User } from 'lucide-react';
...
export const AdminCRM: React.FC = () => {
  ...
  const filteredSubscribers = subscribers.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()).filter(sub => 
    ...
    <AdminLayout>
      <div className="w-full">
        ...
        <div className="mb-8">
           ...
           <p className="text-slate-500 text-base">Manage inquiries, viewings, and newsletter subscribers.</p>
        </div>
        ...
        <div className="flex gap-4 mb-8 border-b border-slate-200">
          <button 
            ...
            className={`pb-4 px-2 text-sm font-bold uppercase tracking-widest transition-colors ${view === 'leads' ? 'text-forge-navy border-b-2 border-forge-gold' : 'text-slate-400 hover:text-forge-navy'}`}
          >
            Sales Leads ({leads.length})
          </button>
          <button 
            onClick={() => setView('subscribers')}
            className={`pb-4 px-2 text-sm font-bold uppercase tracking-widest transition-colors ${view === 'subscribers' ? 'text-forge-navy border-b-2 border-forge-gold' : 'text-slate-400 hover:text-forge-navy'}`}
          >
            ...
            <div className="flex justify-between items-center mb-6">
              <div className="relative w-96">
                ...
                <input 
                  ...
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="w-full pl-10 pr-4 py-3 bg-white border border-slate-200 rounded text-sm focus:outline-none focus:border-forge-gold"
                />
              </div>
              <button 
                onClick={copyEmailsToClipboard}
                className="bg-forge-navy text-white px-6 py-3 rounded font-bold uppercase tracking-widest text-xs hover:bg-forge-dark flex items-center gap-2"
              >
                {copySuccess ? <Check size={16} /> : <Copy size={16} />}
                {copySuccess || 'Copy All Emails'}
              </button>
            </div>
                ...
                <tbody className="text-sm">
                  {filteredSubscribers.length > 0 ? filteredSubscribers.map(sub => (
                    <tr key={sub.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50">
                      <td className="p-5 font-medium text-forge-navy">{sub.email}</td>
                      <td className="p-5 text-slate-500">{new Date(sub.date).toLocaleDateString()}</td>
                    </tr>
                  )) : (
                    <tr>
