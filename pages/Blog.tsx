...
export const Blog: React.FC = () => {
  ...
  // Use dummy posts if none exist for display purposes
      ...
      <div className="relative pt-32 pb-20 bg-slate-900 text-white overflow-hidden">
        ...
        <div className="container mx-auto px-6 relative z-10 text-center">
           ...
           <p className="text-slate-400 max-w-2xl mx-auto text-lg leading-relaxed">
             News, market trends, and curated lifestyle content for the discerning individual.
           </p>
        </div>
      </div>
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12">
             {displayPosts.map(post => (
               <article key={post.id} className="flex flex-col group">
                 <Link to={`/blog/${post.id}`} className="block overflow-hidden mb-6 rounded-sm">
                   {post.coverImage ? (
                 ...
                 <Link to={`/blog/${post.id}`} className="mb-3">
                   <h2 className="text-2xl font-serif text-forge-navy font-bold leading-tight group-hover:text-forge-gold transition-colors">
                     {post.title}
                   </h2>
                 </Link>

                 <p className="text-slate-600 leading-relaxed mb-6 line-clamp-3 flex-grow">
                   {post.excerpt}
                 </p>

                 <Link 
                   to={`/blog/${post.id}`}
                   className="flex items-center gap-2 text-forge-navy font-bold uppercase tracking-widest text-xs hover:text-forge-gold transition-colors self-start"
                 >
